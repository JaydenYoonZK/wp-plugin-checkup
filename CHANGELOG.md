...
- The name seal is now a proper square chop, sitting level with the brand name. Its characters and inner frame are cut clean through the chartreuse field, so whatever the bar shows behind them comes through and the seal reads right in light and dark mode alike. The brand name grew a touch to keep the pair balanced.

## [1.4.13] - 2026-08-16

### Added

- The brand link in the menu bar now carries Jayden's Chinese name, 袁正凯, rendered as a small vertical seal beside the Latin name. It uses the site's own pairing of chartreuse field and dark ink, with an inner frame like a traditional stamp.
...
- The green pill that floated over the status bar on an iPhone turned out to be the skip-to-content link, parked just above the viewport's top edge, exactly the zone iOS paints behind the clock. It parks off to the side now, still sliding in on keyboard focus, and the fix was watched working in the iOS Simulator instead of assumed.

### Changed

- The shield strips from the last two releases are removed: the Simulator settled that nothing inside a page can paint in the status-bar zone, so neither ever drew a pixel. The menu bar is pinned with position fixed and safe-area padding instead, so a hard fling can never lag it a frame, and the faint content visible behind the status bar is Safari's own edge-to-edge design, identical on apple.com itself, so it stays.
...
- On iPhones, scrolling could flash page content in the status-bar strip above the sticky menu. The cover for that strip now lives outside the blurred bar, where iOS actually draws it.

## [1.4.8] - 2026-08-15

### Changed

- The footer's copyright sentence now stands alone, with the changelog-linked version on its own line beneath it.
...
- A Content Security Policy on the browser tool. The only network it needs is the WordPress.org plugin API, so `connect-src` is pinned to `https://api.wordpress.org` and nothing else; your plugin list cannot be sent elsewhere.
